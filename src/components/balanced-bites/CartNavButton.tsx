...
export function CartNavButton() {
  ...
  return (
    <button
      ...
      className={`relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-sm font-semibold transition-[background-color,color,box-shadow,transform] duration-200 ease-out hover:shadow-[0_10px_28px_-16px_rgba(66,98,55,0.45)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#426237]/40 focus-visible:ring-offset-2 focus-visible:ring-offset-[#f4f1eb] active:scale-95 lg:h-11 lg:w-11 ${
        looksActive
    ...
    >
      <span className={bump ? "bb-cart-bump inline-flex" : "inline-flex"}>
        <CartIcon className="h-4 w-4 lg:h-5 lg:w-5" />
      </span>
      {count != null && count > 0 ? (
        <span className="absolute -right-0.5 -top-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-[#ac8058] px-0.5 text-[9px] font-bold text-white lg:-right-1 lg:-top-1 lg:h-5 lg:min-w-5 lg:text-[10px]">
          {label}
        </span>
      ) : null}
    </button>
